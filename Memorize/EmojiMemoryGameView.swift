...
struct EmojiMemoryGameView: View {
    ...
    var body: some View {
        VStack {
            cards.foregroundColor(gameMemory.color)
            HStack {
                score
                Spacer()
                deck.foregroundColor(gameMemory.color)
                Spacer()
                shuffle
            }
            .font(.largeTitle)
    ...
    private var cards: some View {
        AspectVGrid(gameMemory.cards, aspectRatio: aspectRatio) { card in
            if isDealt(card) {
                CardView(card)
    ...
    private var undealCards: [Card] {
        gameMemory.cards.filter { !isDealt($0) }
    }
